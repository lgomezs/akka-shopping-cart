...
public class StreamShoppingcartIT {
    ...
    private static StreamShoppingcartService streamShoppingcartService;
    private static ActorSystem system;
    private static Materializer mat;

    @BeforeClass
    public static void setup() {
        clientFactory = LagomClientFactory.create("integration-test", StreamShoppingcartIT.class.getClassLoader());
        // One of the clients can use the service locator, the other can use the service gateway, to test them both.
        shoppingCartService = clientFactory.createDevClient(ShoppingCartService.class, URI.create(SERVICE_LOCATOR_URI));
        streamShoppingcartService = clientFactory.createDevClient(StreamShoppingcartService.class, URI.create(SERVICE_LOCATOR_URI));
...
//        String answer = await(shoppingCartService.hello("foo").invoke());
//        assertEquals("Hello, foo!", answer);
//        await(shoppingCartService.useGreeting("bar").invoke(new GreetingMessage("Hi")));
//        String answer2 = await(shoppingCartService.hello("bar").invoke());
//        assertEquals("Hi, bar!", answer2);
    ...
    public void helloStream() throws Exception {
        ...
        Source<String, ?> response = await(streamShoppingcartService.directStream().invoke(
